...
public class WordDao {
	...
	public static List<MeaningBean> enQueryZh(String word) throws SQLException {
		...
		return r==null? null : MeaningDao.queryMeaning(r.getWord());
	}
	/**
	...
	public static boolean addWordBean(WordBean word) throws SQLException {
		...
		QueryRunner qr = new QueryRunner();
		return 0 < qr.update(conn, sql, word.getWord(),word.getEg(),word.getEg(),word.getVid());
	}
	/**
	 * 删除单词
	 * @param word
	...
	public static boolean updateWordBean(WordBean word) throws SQLException {
		...
		QueryRunner qr = new QueryRunner();
		return 0 < qr.update(conn, sql, word.getWord(),word.getEg(),word.getEg(),word.getWid());
	}
	/**
	 * 查询某个词汇表里的单词
	 * @param vid
